...
btn.forEach((btn, index) => {
    btn.addEventListener('click', function(){
        ...
        //stop previous song if a diffrent song is selected
...
//define play_song functions to start a song and update the UI
function play_song(){
...
function update_second() {
   ...
   total_duration.textcontext = `${durationMinutes}:${durationSeconds < 10 ? '0' : ''}${durationSeconds}`;
   ...
   //display current playback time 
    ...
    current_duration.textcontext = `${curr_minutes}:${curr_seconds < 10 ? '0' : ''}${curr_seconds}`;
...
down_player.addEventListener('click', function () {
    p_m_player.style.transform = 'translateY(100%)'; //hide popup player
});
...
//add functionallity to forward and back arrows 
